...
NPC npcs[] = {
    // Dziekan jest teraz na Kampusie (2), ale jest NIEWIDZIALNY (BLANK) i stoi za biurkiem
    {"Dziekan", "Dzien dobry. Widze Pana na poprawce... (Wcisnij [1] aby zdawac)", 2, {450, 250}, BLANK}, 
    
    // Ziomek w Parku (1) - teraz on obsluguje sklep
...
// --- ENUMY ---
// DODANO STAN "INTRO" NA POCZĄTKU
enum GameScreen { INTRO, MENU, GAMEPLAY, MINIGAME_FLANKI, MINIGAME_EGZAMIN, KEBAB_MENU, SHOP_MENU, EXAM_MENU, DIALOGUE_MODE };
...
// ZMIANA: GRA ZACZYNA SIĘ OD EKRANU INTRO
...
// NOWA ZMIENNA: Timer dla intro (5 sekund)
...
void SetSpawnPoint(int fromSide) {
    ...
    // 1 = Wchodzimy Z LEWEJ (z pokoju na park) -> lądujemy na lewej ścieżce
    if (fromSide == 1) player.position = { 60, 320 }; 
    
    // 2 = Wchodzimy Z PRAWEJ (z kampusu na park) -> lądujemy na prawej ścieżce
    if (fromSide == 2) player.position = { (float)screenWidth - 80, 320 }; 

    // 3 = Wchodzimy do POKOJU (przy drzwiach z lewej)
    if (fromSide == 3) player.position = { 60, 350 };
    
    // 4 = Wchodzimy na KAMPUS (z parku) -> lądujemy przy dolnym wejściu/lewym
    if (fromSide == 4) player.position = { 50, 400 };
}
...
void WakeUpInRoom(bool hospital) {
    ...
    player.position = { 500, 300 }; // Przy łóżku (prawa strona)
    gameTime = 8.0f;
    dayCount++;
...
void UpdateGame(float dt) {
    ...
    if (currentScreen == INTRO) {
        introTimer -= dt;
        // Jeśli czas minął, przejdź do gry
        if (introTimer <= 0.0f) {
            currentScreen = GAMEPLAY;
        }
        // Podczas intro reszta logiki gry jest pomijana
        return;
    }
    ...
    if (currentScreen == DIALOGUE_MODE) {
        ...
        if (activeNPCIndex != -1 && npcs[activeNPCIndex].name == "Ziomek") {
            if (IsKeyPressed(KEY_S)) currentScreen = SHOP_MENU;
        }
        if (activeNPCIndex != -1 && npcs[activeNPCIndex].name == "Dziekan") {
    ...
    if (currentScreen == GAMEPLAY) {
        gameTime += dt * 0.5f;
        if (gameTime >= 24.0f) { gameTime = 0.0f; dayCount++; }
        player.hunger -= 2.0f * dt;

        if (player.hunger <= 0.0f) {
            player.hunger = 0.0f;
            player.health -= 5.0f * dt; 
            if ((int)(gameTime * 10) % 2 == 0) player.color = RED; else player.color = WHITE;
        } else {
            if (player.speedBoostTimer <= 0) player.color = WHITE;
        }

        if (player.health <= 0.0f) WakeUpInRoom(true); 
    }
    ...
    if(IsKeyPressed(KEY_BACKSPACE)){
        if(phone.state != PHONE_CLOSED){ phone.state = PHONE_CLOSED; phone.gambleInfo = ""; phone.loanInfo = ""; }
    }
    if(phone.state != PHONE_CLOSED && IsKeyPressed(KEY_Q)){
        if(phone.state != MENU1) { phone.state = MENU1; phone.gambleInfo = ""; phone.loanInfo = ""; }
    }
    if(phone.state == PIN){
        int key = GetCharPressed();
        while (key > 0) {
            if ((key >= 48) && (key <= 57) && (phone.inputPin.length() < 4)) phone.inputPin += (char)key;
            key = GetCharPressed();
        }
        if (IsKeyPressed(KEY_BACKSPACE) && phone.inputPin.length() > 0) phone.inputPin.pop_back(); 
        if (IsKeyPressed(KEY_ENTER)) {
             if(phone.inputPin == phone.correctPin){ phone.state = ACCOUNT; phone.inputPin = ""; phone.showError = false; } 
             else { phone.showError = true; phone.inputPin = ""; }
        }
    }
    else if (phone.state == MENU1) {
        if (IsKeyPressed(KEY_RIGHT)) { phone.selectedIcon++; if (phone.selectedIcon > 2) phone.selectedIcon = 0; }
        if (IsKeyPressed(KEY_LEFT)) { phone.selectedIcon--; if (phone.selectedIcon < 0) phone.selectedIcon = 2; }
        if (IsKeyPressed(KEY_ENTER)) {
            if (phone.selectedIcon == 0) phone.state = PIN;
            else if (phone.selectedIcon == 1) { phone.state = APP_CRYPTO; phone.currentBet = 10; }
            else if (phone.selectedIcon == 2) { phone.state = APP_LOANS; phone.loanOption = 0; phone.loanInfo = ""; }
        }
    }
        ...
        if (IsKeyPressed(KEY_ENTER) && player.money >= phone.currentBet) {
            if (GetRandomValue(0, 100) >= 50) { player.money += phone.currentBet; phone.gambleInfo = "Wygrana!"; phone.infoColor = GREEN; stats.GainXP(5.0f); } 
            else { player.money -= phone.currentBet; phone.gambleInfo = "Przegrana..."; phone.infoColor = RED; }
        }
    }
        ...
        if (IsKeyPressed(KEY_ENTER)) {
            if (phone.loanOption == 0) { player.money += 100.0f; player.debt += 120.0f; phone.loanInfo = "Wzieles 100 zl!"; }
                ...
                if (player.debt > 0 && player.money > 0) {
                    if (player.money >= player.debt) { player.money -= player.debt; player.debt = 0; phone.loanInfo = "Splacono!"; } 
                    else { player.debt -= player.money; player.money = 0; phone.loanInfo = "Splaciles czesc."; }
                } else phone.loanInfo = "Brak kasy/dlugu.";
            }
        }
    }
    ...
    if (currentScreen == MINIGAME_FLANKI) {
        ...
        if (flanki.state == AIMING) {
            float speed = 1.5f * dt;
            if (flanki.powerGoingUp) flanki.power += speed; else flanki.power -= speed;
            ...
            if (CheckCollisionCircleRec({ flanki.ballPos.x + 10, flanki.ballPos.y + 10 }, 10, { 700, 400, 60, 100 })) {
                flanki.playerWon = true; flanki.score++; stats.GainXP(10.0f); flanki.state = RESULT;
            }
            if (flanki.ballPos.y > 500) { flanki.playerWon = false; flanki.state = RESULT; }
        }
        else if (flanki.state == RESULT) { if (IsKeyPressed(KEY_SPACE)) ResetFlanki(); }
        return;
    }
    ...
    if (currentScreen == MINIGAME_EGZAMIN) {
        if (player.hasCheatSheet && IsKeyPressed(KEY_SPACE)) {
            ...
            player.money += reward; stats.GainXP(activeExamTier * 20.0f); player.hasCheatSheet = false; currentScreen = GAMEPLAY; return;
        }
        if (IsKeyPressed(examQuestions[currentQuestionIndex].correctKey)) {
            float reward = (activeExamTier == 1) ? 50.0f : (activeExamTier == 2 ? 150.0f : 500.0f);
            player.money += reward; stats.GainXP(activeExamTier * 20.0f); currentScreen = GAMEPLAY;  
        } 
        else if (GetKeyPressed() != 0) currentScreen = GAMEPLAY;
        return; 
    }

    if (currentScreen == KEBAB_MENU) {
        if (IsKeyPressed(KEY_Q)) currentScreen = GAMEPLAY; 
        if (IsKeyPressed(KEY_DOWN)) kebabOption++; if (IsKeyPressed(KEY_UP)) kebabOption--;
        if (kebabOption > 2) kebabOption = 0; if (kebabOption < 0) kebabOption = 2;
        if (IsKeyPressed(KEY_ENTER)) {
            if (kebabOption == 0 && player.money >= 8.0f) { player.money -= 8.0f; player.hunger += 20.0f; currentScreen = GAMEPLAY; }
            else if (kebabOption == 1 && player.money >= 18.0f) { player.money -= 18.0f; player.hunger += 50.0f; currentScreen = GAMEPLAY; }
            else if (kebabOption == 2 && player.money >= 30.0f) { player.money -= 30.0f; player.hunger = 100.0f; player.health += 30.0f; currentScreen = GAMEPLAY; }
    ...
    if (currentScreen == SHOP_MENU) {
        if (IsKeyPressed(KEY_Q)) currentScreen = GAMEPLAY;
        if (IsKeyPressed(KEY_ONE) && player.money >= 20.0f) { player.money -= 20.0f; player.speedBoostTimer = 20.0f; }
        if (IsKeyPressed(KEY_TWO) && player.money >= 100.0f && !player.hasCheatSheet) { player.money -= 100.0f; player.hasCheatSheet = true; }
        return;
    }
    ...
    if (currentLocation == POKOJ) {
        ...
        // Wyjście do Parku - DRZWI W GÓRNYM LEWYM ROGU
...
void DrawGame() {
    ...
    // --- RYSOWANIE EKRANU INTRO ---
    if (currentScreen == INTRO) {
        // Rysujemy teksturę intro rozciągniętą na cały ekran
        DrawTexturePro(texturaIntro, 
                       {0.0f, 0.0f, (float)texturaIntro.width, (float)texturaIntro.height}, 
                       {0.0f, 0.0f, (float)screenWidth, (float)screenHeight}, 
                       {0.0f, 0.0f}, 0.0f, WHITE);
        
        // Opcjonalnie: pasek ładowania lub licznik
        // DrawText(TextFormat("Start za: %.1f", introTimer), 20, screenHeight - 40, 20, WHITE);
    }
    else if (currentScreen == MINIGAME_FLANKI) {
        ...
        if (flanki.state == AIMING) {
            ...
            if (flanki.playerWon) DrawText("TRAFIONY!", 300, 200, 40, GOLD); else DrawText("PUDLO!", 300, 200, 40, MAROON);
        }
        stats.DrawHUD();
    } 
        ...
        if (player.hasCheatSheet) DrawText("SCIAGA DOSTEPNA [SPACJA]", 100, 550, 20, GOLD);
    }
    else {
        // --- ŚWIAT ---
        if (currentLocation == POKOJ) {
            ...
            if (gameTime >= 20.0f || gameTime < 6.0f) DrawRectangle(0, 0, screenWidth, screenHeight, Fade(BLACK, 0.5f)); 
            else if (gameTime >= 18.0f) DrawRectangle(0, 0, screenWidth, screenHeight, Fade(ORANGE, 0.2f)); 
        ...
        float barWidth = 220.0f; float barHeight = 20.0f;
        float x = screenWidth - barWidth - 20; float y = 50.0f; 
        ...
        y += 25;
        DrawRectangle(x, y, barWidth, barHeight, Fade(BLACK, 0.6f));
        DrawRectangle(x, y, barWidth * (player.hunger / 100.0f), barHeight, (player.hunger < 20.0f) ? ORANGE : GREEN);
        DrawRectangleLines(x, y, barWidth, barHeight, WHITE);
        DrawText("GLOD", x + 5, y + 2, 10, WHITE);

        if (notificationTimer > 0) DrawText(notification.c_str(), screenWidth/2 - MeasureText(notification.c_str(), 30)/2, 100, 30, RED);
        ...
        // --- TELEFON (POPRAWIONY DESIGN) ---
        if (phone.state != PHONE_CLOSED) {
            int px = 550, py = 150;
            int pw = 240, ph = 420;
            
            // Obudowa (Smartphone look)
            DrawRectangleRounded({(float)px, (float)py, (float)pw, (float)ph}, 0.1f, 10, DARKGRAY);
            // Używamy wersji 4-argumentowej dla kompatybilności
            ...
            if (phone.state == MENU1) {
                ...
                DrawRectangle(px + 10, py + 40, pw - 20, ph - 60, BLACK); // Tryb ciemny dla krypto
